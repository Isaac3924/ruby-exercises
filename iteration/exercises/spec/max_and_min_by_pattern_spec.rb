RSpec.describe 'max and min by pattern' do
  ...
  it 'test 3' do
    ...
    meals.each do |meal|
      if shortest_word.length > meal.length
        shortest_word = meal
      end
    end
  ...
  it 'test 4' do
    ...
    meals.each do |meal, dish|
      if shortest_word.length > dish.length
        shortest_word = dish
      end
    end
  ...
  it 'test 8' do
    ...
    ages.each do |title, years|
      if oldest[:age] < years
        oldest[:name] = title.to_s
        oldest[:age] = years
      end
    end
  ...
  it 'test 9' do
    ...
    programmers.each do |array|
      if fewest_programmers.length > array.length
        fewest_programmers = array
      end
    end
  ...
  it 'test 10' do
    ...
    programmers.each do |key_programminglanguage_symbol, value_name_array|
      if programmers.fetch(fewest_programmers).size > value_name_array.size
        fewest_programmers = key_programminglanguage_symbol
      end
    end
